...
\minpurp{O-Notation}
$g(n) = O(f(n)) \Leftrightarrow \left(g(n)\frac{1}{f(n)}\right)$ ist beschraenkt (z.B. Konvergent)\\
...
\mathitem{ x_1 = b,  \\
 x_n = a_nx_{n-1} +b_n  }
\mathitem{\pi_n = \prod_{i=2}^na_i, \pi_1=1          }
...
\halfpage{
\begin{itemize}
\mathitem{ \sum_{i=1}^i \frac{1}{n} = H_n~~~~Abschatzung: ln(n+1) <= H_n <= ln(n)+1          \\
...
\mathitem{\sum_{i=1}^n x_i^i = \frac{x^{n+1}-1}{x-1}\\
\sum_{i=1}^n ix_{i-1}^i = \frac{(n+1)x^n(x-1)-(x^{n+1}-1)}{(x-1)^2}}
\end{itemize}
}
