...
\node[attribute](proj){\proj};
\node[attribute](selk)[below of=proj]{\selk}edge(proj);
\node[attribute](selor)[below of=selk]{\selor}edge(selk);
\node[attribute](joinor)[below of=selor]{\joina}edge(selor);
\node[attribute](joink)[below left of=joinor]{\joink}edge(joinor);
\node[attribute](k)[below left of=joink]{Kunde k}edge(joink);
\node[attribute](a)[below right of=joink]{Auftrag a}edge(joink);
\node[attribute](pos)[below right of=joinor]{Position pos}edge(joinor);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=4em]
\node[attribute](proj){\proj};
\node[attribute](joink)[below of=proj]{\joink}edge(proj);
\node[attribute](pos)[below left of=joink]{Position pos}edge(joink);
\node[attribute](selor)[below right of=joink]{\selor}edge(joink);
\node[attribute](joinor)[below of=selor]{\joina}edge(selor);
\node[attribute](selk)[below right of=joinor]{\selk}edge(joinor);
\node[attribute](a)[below of=selk]{Auftrag a}edge(selk);
\node[attribute](k)[below left of=joinor]{Kunde k}edge(joinor);
\end{tikzpicture}
